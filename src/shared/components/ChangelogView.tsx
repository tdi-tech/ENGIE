...
const changelogData: { id: string; version: string; date: string; tag: string; title: string; changes: { id: string; type: string; text: string }[] }[] = [
    {
        ...
        changes: [
            ...
            { id: 'd4', type: 'divider', text: '' },
            { id: 's5', type: 'title', text: 'Navegación y Layout Móvil' },
            { id: 'c15', type: 'ui', text: 'Menú de hamburguesa en móvil: el botón ejecutaba siempre setSidebarOpen(true), de modo que una vez desplegado el menú el segundo toque no lo ocultaba (React no vuelve a renderizar al asignar el mismo valor al estado). Ahora alterna abierto/cerrado, cambia su icono a X mientras el menú está desplegado y expone el estado para accesibilidad (aria-expanded, aria-label dinámico y aria-controls sobre el id app-sidebar del panel lateral). Se conserva el cierre al tocar el fondo oscuro y el cierre automático al navegar a otra vista.' },
        ],
    },
];
