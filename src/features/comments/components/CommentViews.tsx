                                ...
                                {getNormalizedComments(selectedComment).map((c: any, idx: number) => (
                                    <div key={c.id || idx} className={`p-4 theme-bg-low rounded-xl border space-y-3 ${c.sentiment === 'Negativo' ? 'border-red-500/30 bg-red-500/5' : 'theme-border'}`}>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {c.sentiment && (c.fuenteMonitoreo === 'Medios digitales'
                                                ? <span title="Sentimiento de la Mención" className={`px-2.5 py-1 text-[10px] font-bold rounded-md uppercase tracking-wider ${getSentimentDot(c.sentiment).badge}`}><span className={`inline-block w-2 h-2 rounded-full mr-1 ${getSentimentDot(c.sentiment).dot}`}></span>Sentimiento: {c.sentiment}</span>
                                                : <span title="Sentimiento de la Mención" className="px-2.5 py-1 text-[10px] font-bold rounded-md uppercase tracking-wider bg-blue-500/10 text-blue-700 dark:text-blue-300 border border-blue-500/30">Sentimiento: {c.sentiment}</span>)}
