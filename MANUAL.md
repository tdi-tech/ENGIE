...
8. La pestaña **Menciones** del Dashboard incorpora la analítica **Top Actores / Fuentes Recurrentes**: un **radar** (`chart.js`) con los **10 actores o fuentes de más menciones registradas** y, al lado, el **ranking con el número concreto de menciones** y su porcentaje sobre el total (hasta 10 posiciones, con el resumen de los restantes). El radar traza exactamente el mismo Top 10 que lista el ranking (`RADAR_TOP = RANKING_TOP`), y con más de 6 ejes compacta etiquetas y puntos y amplía el lienzo para mantener la legibilidad. Agrupa con la misma clave analítica del módulo de *Reportes y Analítica* —el campo **Usuario o Sitio Web** reducido a etiqueta corta `@usuario` o dominio— y respeta el tema claro/oscuro con los tokens corporativos.
