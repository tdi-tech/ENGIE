...
8. La pestaña **Menciones** del Dashboard incorpora la analítica **Top Actores / Fuentes Recurrentes**: un **gráfico de barras horizontales** (`chart.js`) con los **10 actores o fuentes de más menciones registradas** y, al lado, el **detalle numérico con el conteo concreto de menciones** y su porcentaje sobre el total (hasta 10 posiciones, con el resumen de los restantes). El gráfico y la lista comparten el mismo límite (`TOP_ACTORES = RANKING_TOP`), las barras se ordenan de mayor a menor con intensidad decreciente y la altura del lienzo se ajusta al número de barras. Se sustituyó el **radar** anterior porque, al crecer el número de ejes, las etiquetas se solapaban y la lectura del polígono se volvía confusa. Agrupa con la misma clave analítica del módulo de *Reportes y Analítica* —el campo **Usuario o Sitio Web** reducido a etiqueta corta `@usuario` o dominio— y respeta el tema claro/oscuro con los tokens corporativos.
